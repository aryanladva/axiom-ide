...
- Do not replace private `packages/opencode` imports with `@axiom-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@axiom-ai/tui`.
...
Status: Completed. The private `@axiom-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@axiom-ai/tui`.
...
- The package has no dependency on `opencode`, `@axiom-ai/cli`, or
  `@axiom-ai/core`.
...
primitives now live in `@axiom-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@axiom-ai/tui/runtime`. Movable app, component,
...
`@axiom-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
canonical in `@axiom-ai/tui`. Configured references resolve through the new
...
`@axiom-ai/tui`. The legacy host injects a narrow plugin host that retains
...
public `@axiom-ai/tui` root while retaining worker/server/config/plugin and
...
- Narrow `@axiom-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@axiom-ai/core`, or either
  executable package.
...
rg '@axiom-ai/core|packages/opencode|packages/cli' packages/tui
