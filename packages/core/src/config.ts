...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const names = ["axiom.json", "axiom.jsonc", "opencode.json", "opencode.jsonc"]
    ...
    const discovered = locationIsGlobal
      ...
      : yield* fs
          .up({
            targets: [".axiom", ".opencode", ...names.toReversed()],
    ...
    const directories = [
      globalDirectory,
      ...discovered
        .filter((item) => path.basename(item) === ".axiom" || path.basename(item) === ".opencode")
        .toReversed()
