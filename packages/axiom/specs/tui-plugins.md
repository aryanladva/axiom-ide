...
- Author package entrypoint is `@axiom-ai/plugin/tui`.
...
- Import types from `@axiom-ai/plugin/tui`.
- `@axiom-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@axiom-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@axiom-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@axiom-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@axiom-ai/plugin/tui`.
