...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export AXIOM_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AXIOM_CLIENT=desktop",
    `export AXIOM_SERVER_USERNAME=${shellEscape(username)}`,
    `export AXIOM_SERVER_PASSWORD=${shellEscape(password)}`,
