...
const APP_IDS: Record<string, string> = {
  dev: "ai.axiom.desktop.dev",
  beta: "ai.axiom.desktop.beta",
  prod: "ai.axiom.desktop",
}
const TEST_ONBOARDING = process.env.AXIOM_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.AXIOM_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.AXIOM_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.axiom.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `axiom-onboarding-${randomUUID()}`)
    rmSync(root, { recursive: true, force: true })
    ;["data", "config", "cache", "state", "desktop", "session"].forEach((dir) =>
      mkdirSync(join(root, dir), { recursive: true }),
    )
    process.env.AXIOM_DB = ":memory:"
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "AXIOM Dev")
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.AXIOM_PORT
      if (fromEnv) {
